...
personclass = [[-1 for y in range(m)] for x in range(n)]
visitpos = [[-1 for y in range(m)] for x in range(n)]
...
# if i-a >= 0 and codepos[i-a][j] < 7 and codepos[i-a][j] >= 0 and visitpos[i-a][j] == -1:
#             visitpos[i-a][j] == 0
#             i = i - a
#             return i,j,1
            
#         if j -a >= 0 and codepos[i][j-a] < 7 and codepos[i][j-a] >= 0 and visitpos[i][j-a] == -1:
#             visitpos[i][j-a] == 0
#             j = j - a
#             return i,j,1
            
#         if i+a < n and codepos[i+a][j] < 7 and codepos[i+a][j] >=0 and visitpos[i+a][j] == -1:
#             visitpos[i+a][j] == 0 
#             i = i + a
#             return i,j,1
            
#         if j+a < m and codepos[i][j+a] < 7 and codepos[i][j+a] >= 0 and visitpos[i][j+a] == -1:
#             visitpos[i][j+a] == 0
#             j = j + a
#             return i,j,1
            
#         if i+a < n and j-a >= 0 and codepos[i+a][j-a] < 7 and codepos[i+a][j-a] >= 0 and visitpos[i+a][j-a] == -1:
#             visitpos[i+a][j-a] == 0
#             i = i + a
#             j = j - a
#             return i,j,1
                
#         if i-a>=0 and j+a < m and codepos[i-a][j+a] < 7 and codepos[i-a][j+a] >= 0 and visitpos[i-a][j+a] == -1:
#             visitpos[i-a][j+a] == 0
#             i = i - a
#             j = j + a
#             return i,j,1
                
#         if i+a < n and j+a < m and codepos[i+a][j+a] < 7 and codepos[i+a][j+a] >= 0 and visitpos[i+a][j+a] == -1:
#             visitpos[i+a][j+a] == 0
#             i = i + a
#             j = j + a
#             return i,j,1
                
#         if i-a >= 0 and j - a>= 0 and codepos[i-a][j-a] < 7 and codepos[i-a][j-a] >= 0 and visitpos[i-a][j-a] == -1:
#             visitpos[i-a][j-a] == 0
#             i = i - a
#             j = j - a
#             return i,j,1
...
#         p,q,k = self.find_nearest(i-a,j)
#         if k == 0:
#             p,q,k = self.find_nearest(i,j-a)
#         if k == 0:
#             p,q,k = self.find_nearest(i+a,j)
#         if k == 0:
#             p,q,k = self.find_nearest(i,j+a)
#         if k == 0:
#             p,q,k = self.find_nearest(i+a,j-a)
#         if k == 0:
#             p,q,k = self.find_nearest(i-a,j+a)
#         if k == 0:
#             p,q,k = self.find_nearest(i+a,j+a)
